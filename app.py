...
# ============ AI 文献检索 ============
def ai_search(question, doc_text=None, doc_filename=None):
    """AI 自动拆解问题，多关键词搜索，获取完整摘要，结合文档内容汇总结果"""
    from langchain_openai import ChatOpenAI
...
{result['abstract']}"""
    ...
    # 构建系统提示词
    if doc_text:
        # 截断文档内容，避免超出 token 限制（取前 8000 字符）
...
用户上传了文档「{doc_filename}」，并提出了研究问题。你需要：
1. 先理解上传文档的内容
2. 再检索相关学术文献
3. 综合文档内容 + 检索到的文献，给出回答
...
4. 【必须】从搜索结果中挑选最相关的3-5篇论文，用 tool_get_abstract 获取它们的完整摘要
   - 至少获取3篇论文的完整摘要
5. 基于【上传文档内容 + 论文完整摘要 + 标题 + 你的专业知识】，综合回答用户问题

【回答格式要求】
- 先说明文档中与问题相关的内容，标注"（来自上传文档）"
...
def main():
    ...
    tab1, tab2 = st.tabs(["🔍 文献检索（可上传文档）", "📄 文档问答（仅文档）"])
    
    # ==================== 标签页 1：文献检索 + 文档上传 ====================
    with tab1:
        st.subheader("智能文献检索", anchor=False)
        st.write("上传文档后，AI 会结合文档内容和检索到的文献综合回答你的问题")
        
        # 文档上传区域
        uploaded_file = st.file_uploader(
            "📎 可选：上传文档（PDF/TXT/DOCX），AI 会结合文档内容检索和回答",
            type=["pdf", "txt", "docx"],
            accept_multiple_files=False,
            key="search_doc_upload"
        )
        
        # 用 session_state 保存文档内容
        if "search_doc_text" not in st.session_state:
            st.session_state.search_doc_text = None
            st.session_state.search_doc_name = None
        
        if uploaded_file:
            if st.session_state.search_doc_name != uploaded_file.name:
                with st.spinner(f"正在加载 {uploaded_file.name}..."):
                    doc_text, msg = load_document_text(uploaded_file)
                    if doc_text:
                        st.session_state.search_doc_text = doc_text
                        st.session_state.search_doc_name = uploaded_file.name
                        st.success(msg)
                    else:
                        st.error(msg)
        
        # 显示已加载的文档
        if st.session_state.search_doc_name:
            col1, col2 = st.columns([3, 1])
            with col1:
                st.info(f"📄 已加载文档：{st.session_state.search_doc_name}（{len(st.session_state.search_doc_text)} 字符）")
            with col2:
                if st.button("🗑️ 移除文档"):
                    st.session_state.search_doc_text = None
                    st.session_state.search_doc_name = None
        ...
        question = st.text_area("输入你的研究问题", height=80,
                               placeholder="例如：为什么碳酸氢钠浸出液除硅效果比钒渣钠化焙烧浸出液差？结合我上传的文档分析")
        ...
        if search_btn and question.strip():
            with st.spinner("AI 正在分析文档、检索文献并获取完整摘要...（约30-60秒）"):
                answer, error = ai_search(
                    question.strip(),
                    doc_text=st.session_state.search_doc_text,
                    doc_filename=st.session_state.search_doc_name
                )
                if error:
                    st.error(error)
                else:
