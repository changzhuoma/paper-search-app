...
def load_document_text(uploaded_file):
    ...
    try:
        ...
        return full_text, f"✅ 已加载「{uploaded_file.name}」，共 {len(docs)} 页/段"
    except Exception as e:
...
def ai_search(question, docs_dict=None):
    """
    AI 自动拆解问题，多关键词搜索，获取完整摘要，结合多个文档内容汇总结果
    docs_dict: {文件名: 文档内容}
    """
    from langchain_openai import ChatOpenAI
...
用户上传了 {len(docs_dict)} 个文档：{doc_names}
并提出了研究问题。你需要：
1. 先理解所有上传文档的内容
2. 再检索相关学术文献
3. 综合所有文档内容 + 检索到的文献，给出回答
...
4. 【必须】从搜索结果中挑选最相关的3-5篇论文，用 tool_get_abstract 获取它们的完整摘要
   - 至少获取3篇论文的完整摘要
5. 基于【所有上传文档内容 + 论文完整摘要 + 标题 + 你的专业知识】，综合回答用户问题
...
def main():
    ...
    # ==================== 标签页 1：文献检索 + 多文档上传 ====================
    with tab1:
        st.subheader("智能文献检索（支持上传多个文档）", anchor=False)
        st.write("一次上传多个文档，AI 会综合所有文档内容和检索到的文献回答你的问题")
        ...
        if search_btn and question.strip():
            with st.spinner("AI 正在分析文档、检索文献并获取完整摘要...（约30-60秒）"):
