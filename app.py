...
def load_document_text(uploaded_file):
    ...
    try:
        ...
        return full_text, f"✅ 已加载「{uploaded_file.name}」，共 {len(docs)} 页/段，{len(full_text)} 字符"
    except Exception as e:
...
# ============ AI 文献检索 ============
def ai_search(question, docs_dict=None):
    """
    AI 自动拆解问题，多关键词搜索，获取完整摘要，结合多个文档全文汇总结果
    docs_dict: {文件名: 文档内容}
    """
    from langchain_openai import ChatOpenAI
...
{result['abstract']}"""
    ...
    # 构建系统提示词，根据是否上传文档动态调整
    if docs_dict and len(docs_dict) > 0:
        # 【关键修改】传全文，不截断
        # 每个文档最多 15000 字符（约10页论文），超过才截断并提示
        all_doc_text = ""
        total_chars = 0
        for fname, ftext in docs_dict.items():
            if len(ftext) > 15000:
                excerpt = ftext[:15000] + "\n...（文档较长，已截断，仅展示前15000字符）"
            else:
                excerpt = ftext
            all_doc_text += f"\n{'='*60}\n【文档：{fname}】（全文，共{len(ftext)}字符）\n{'='*60}\n{excerpt}\n"
            total_chars += len(ftext)
        
        doc_names = "、".join(docs_dict.keys())
        
        system_prompt = f"""你是学术文献检索助手，同时也是文档分析专家。

用户上传了 {len(docs_dict)} 个文档：{doc_names}（共约 {total_chars} 字符）
并提出了研究问题。你需要：
1. 【重要】完整阅读所有上传文档的全文，不要只看摘要和引言，要重点关注：
   - 实验方法与工艺条件
   - 实验结果与数据
   - 讨论与分析
   - 结论
2. 再检索相关学术文献
3. 综合所有文档全文内容 + 检索到的文献，给出回答

【执行步骤】
1. 拆解用户问题，提取3-5个关键概念，翻译成英文
2. 生成3-5组不同角度的英文关键词
3. 用 tool_search 分别搜索（每次一个关键词）
4. 【必须】从搜索结果中挑选最相关的3-5篇论文，用 tool_get_abstract 获取它们的完整摘要
   - 至少获取3篇论文的完整摘要
5. 基于【所有上传文档全文 + 论文完整摘要 + 标题 + 你的专业知识】，综合回答用户问题

【回答格式要求】
- 先说明文档中与问题相关的内容，标注"（来自文档：文件名）"
- 要引用文档中的具体实验数据、工艺条件、结果，不要泛泛而谈
- 再说明检索到的文献中的相关结论，用 [1] [2] 标注文献编号
- 如果不同文档之间有矛盾，指出差异并分析
- 如果文档内容和文献结论一致，说明"文档与文献相互印证"
...
重要：
1. 不获取完整摘要就回答是不合格的，必须先调用 tool_get_abstract
2. 必须阅读文档全文，不要只看前面的摘要和引言"""
    else:
...
def main():
    ...
    # 检查 API Key 是否配置（只在后台检查，不显示给用户）
    deepseek_key = get_deepseek_key()
    elsevier_key = get_elsevier_key()
    
    if not deepseek_key or not elsevier_key:
        st.error("⚠️ 系统 API Key 未配置，请联系管理员")
        return
    ...
    with tab1:
        ...
        st.write("一次上传多个文档，AI 会阅读文档全文并综合检索到的文献回答你的问题")
        
        # 用 session_state 保存多个文档内容：{文件名: 内容}
        if "search_docs" not in st.session_state:
            st.session_state.search_docs = {}
        
        # 多文件上传
        uploaded_files = st.file_uploader(
            "📎 可选：上传多个文档（PDF/TXT/DOCX，可多选）",
            type=["pdf", "txt", "docx"],
            accept_multiple_files=True,
            key="search_multi_upload"
        )
        
        # 加载新上传的文档
        if uploaded_files:
            for f in uploaded_files:
                if f.name not in st.session_state.search_docs:
                    with st.spinner(f"正在加载 {f.name}..."):
                        doc_text, msg = load_document_text(f)
                        if doc_text:
                            st.session_state.search_docs[f.name] = doc_text
                            st.success(msg)
                        else:
                            st.error(msg)
        
        # 显示已加载的文档列表
        if st.session_state.search_docs:
            st.info(f"📄 已加载 {len(st.session_state.search_docs)} 个文档：")
            for i, (fname, ftext) in enumerate(st.session_state.search_docs.items(), 1):
                col1, col2 = st.columns([4, 1])
                with col1:
                    st.write(f"  {i}. {fname}（{len(ftext)} 字符）")
                with col2:
                    if st.button(f"移除", key=f"remove_{fname}"):
                        del st.session_state.search_docs[fname]
                        st.rerun()
        ...
        question = st.text_area("输入你的研究问题", height=80,
                               placeholder="例如：结合我上传的文档，分析为什么碳酸氢钠浸出液除硅效果比钒渣钠化焙烧浸出液差？")
        ...
        if search_btn and question.strip():
            with st.spinner("AI 正在阅读文档全文、检索文献并获取完整摘要...（约30-60秒）"):
                answer, error = ai_search(
                    question.strip(),
                    docs_dict=st.session_state.search_docs if st.session_state.search_docs else None
                )
                if error:
                    st.error(error)
                else:
    ...
    # ==================== 标签页 2：纯文档问答 ====================
    with tab2:
        st.subheader("文档问答（RAG，仅基于文档）", anchor=False)
        st.write("上传 PDF/TXT/DOCX 文档，仅基于文档内容回答问题")
